...
```
For example, submitting the following API request will download all the records in the ‘sample’ node of the project ‘GEO-GSE63878’ in [the demo data commons][Gen3 DC] as a tab-separated values file (TSV):
```
...
{json/tsv} is the format in which data will be downloaded, either 'json' or 'tsv'.
```
For example, submitting the following API request will download the two records corresponding to the UUIDs (the ‘id’ property) ‘180554c0-d0e1-41f2-b5b0-47655d7975ed’ and ‘3e54268c-b4a6-4cf8-bedc-1e9e49f9d6e9’, which are records in the ‘subject’ node of the project ‘GEO-GSE63878’ in the [demo data commons][Gen3 DC]. This downloads a ‘subjects.tsv’ file containing those two records.
```
...
[microservice docs]: https://gen3.org/resources/developer/microservice
...
<!-- Credentials -->
[img API key profile]: https://gen3.org/resources/user/using-api/API_key_profile_page.png
[img API copy keys]: https://gen3.org/resources/user/using-api/API_copy_keys.png
